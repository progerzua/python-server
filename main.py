...
class Server:
    server = str.encode("\nServer: SimpleHTTP-clone/0.1b " + sys.version.split()[0])
    ...
    def start_server(self):
            ...
            while True:
                ...
                if (method == b'GET'):
                    ...
                    if(isfolder and valid_path and self.scan_index(path)):
                        ...
                        dirs, files = self.scandir(path)
                        body = self.generate_response(dirs, files, path)

                        content_type = b"\nContent-type: text/html; charset=utf-8"
                        content_length = b"\nContent-Length: " + str.encode(str(len(body)))
                        status = b"HTTP/1.1 200 OK"
                    elif(valid_path and not isfolder):
                        content_type, content_length, body = self.serve_file(path)
                        status = b"HTTP/1.1 200 OK"
                    elif(not valid_path):
                        body = b'''
                ...
                HEADERS = status + self.server + content_type + content_length
    ...
    def generate_response(self, dirs, files, path):
        if path == "":
            foldername = b""
        else:
